...
class GridRetry(object):
  ...
  def __init__(self,
    ...
    github_actions:bool=True,chain:bool = False,
    # used in status
  ...
  def status_to_csv(self, head:int = None) -> List[List]:
    """Format grid output to csv output"""
    csv = []
    n=0
    for l in self._po.stdout.decode("utf-8").splitlines():
  ...
  def set_status_param(self, obj_type:str, obj_id_col:str, obj_status_col:str, obj_id_exp:str, obj_status_exp:str):
    ...
    if (self.chain == True):
      ...
      print('| '.join(map(str, self.search_params)))
  ...
  def cli(self, cmd:str):
    ...
    # _po might not be defined on abend
    self._po = None
    while self.total_retry_cnt < self.max_total_retry_cnt:
      self.total_retry_cnt += 1
      try:
        self._po = subprocess.run(args, cwd=self.cwd, capture_output=True, shell=True, timeout=self.cmd_exec_timeout) 
        if self._po.returncode == 0:
          break
        else:
          self.cmd_errs_cnt += 1
          logging.info(f"{args}:total_retry_cnt {self.total_retry_cnt}/{self.max_total_retry_cnt}:cmd_errs_cnt {self.cmd_errs_cnt}/{self.max_cmd_errs_cnt} exited with {self._po.returncode}")
          logging.info(self._po.stderr.decode("utf-8"))
          logging.info(self._po.stdout.decode("utf-8")) 
  ...
  def status_summary(self,cmd:str, obj_type:str, obj_id:str):
    ...
    while self.total_retry_cnt < self.max_total_retry_cnt:
      ...
      if (self.search_params[0].match_cnt == 0):
        ...
        raise Exception("Should have never gotten heret")
